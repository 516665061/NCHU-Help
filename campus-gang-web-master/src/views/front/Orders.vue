  ...
  <el-container style="height: calc(100vh - 60px);">
    ...
    <el-main>
      <div class="content-list">
        <el-card class="box-card" v-for="item in orders" @click.native.stop="detail(item.id)">
    ...
    <el-dialog title="发布订单" :visible.sync="dialogFormVisible" top="0vh" size="mini">
      ...
      <div style="color: black;display: flex; justify-content: center">
        需支付
        <span style="margin-left: 10px; color: red">{{form.price + (form.price * 0.1 < 1 ? form.price * 0.1:1)}}元</span>
        <el-tooltip class="item" effect="dark" content="收取订单金额10%，最高1元" placement="right-start">
          <span style="margin-left: 10px; color: #AAA;">含平台手续费{{(form.price * 0.1 < 1 ? form.price * 0.1:1)}}元<i class="el-icon-warning-outline"></i></span>
        </el-tooltip>
      </div>
    ...
    onSubmit() {
      ...
      this.$refs.form.validate((valid) => {
        if (valid) {
          ...
          this.form.serviceCharge = this.form.price * 0.1 < 1 ? this.form.price * 0.1:1
